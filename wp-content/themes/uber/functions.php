...
function ttrust_scripts() {	
	...
	if ( (is_front_page() && of_get_option('ttrust_bkg_slideshow_enabled') && !is_mobile()) || (has_single_custom_background() &&  !$is_tiled_bkg)) :	
	...
	if ( (is_front_page() && of_get_option('ttrust_bkg_slideshow_enabled') && (!is_mobile()) || has_single_custom_background()) ) :	
...
<?php if ($call_to_action_font) : ?>
	#callToAction p { font-family: '<?php echo $call_to_action_font; ?>'; }
<?php endif; ?>
...
<?php
//Disable the WordPress custom background on the home page if background slideshow is enabled
if ( (is_front_page() && of_get_option('ttrust_bkg_slideshow_enabled')) && !is_mobile() ) : ?>	
...
function ttrust_footer() {		
	...
	if(is_front_page()){
		if (of_get_option('ttrust_bkg_slideshow_enabled') && !is_mobile()) {	
			include(TEMPLATEPATH . '/js/background_home.php');			
		}
...
$meta_box_groups = array($project_details, $home_slideshow_options, $page_options, $portfolio_options, $background_options);
...
function create_meta_box() {	
	global $project_details, $home_slideshow_options, $page_options, $portfolio_options, $background_options;	
	...
	if ( function_exists('add_meta_box') ) {
		...
		add_meta_box( 'new-meta-boxes-home-slideshow', __('Home Slideshow Options','themetrust'), 'new_meta_box', 'project', 'normal', 'high', array('inputs'=>$home_slideshow_options) );
